"""
RainSense — Model Training Script
Trains a HistGradientBoosting classifier + regressor on the Australian weather dataset
and saves the model bundle to rain_model_bundle.pkl.
...
def build_dataset():
    """Load and engineer features from the raw CSV."""
...
def main():
    ...
    print(f"Dataset: {len(X):,} rows | Rain rate: {y_rain.mean():.1%}")
    ...
    regressor.fit(X_train, amt_train)

    # Evaluate
    probs   = classifier.predict_proba(X_test)[:, 1]
    preds   = (probs >= 0.5).astype(int)
    rain_preds = regressor.predict(X_test).clip(min=0)

    metrics = {
        "accuracy":         round(float(accuracy_score(y_test, preds)), 4),
        "roc_auc":          round(float(roc_auc_score(y_test, probs)), 4),
        "rainfall_mae_mm":  round(float(mean_absolute_error(amt_test, rain_preds)), 4),
